...
BOOST_AUTO_TEST_CASE(create_instance)
{
  ...
  boost::shared_ptr< Evaluator >
          evaluator_ = boost::make_shared< Evaluator >(1);
  ...
  light_constructor_left =
          [&sensor_left, light_constructor_common](
                  std::vector< float > coordinates)
                  -> revolve::brain::FakeLightSensor *
          {
            delete sensor_left;
  ...
  light_constructor_right =
          [&sensor_right, light_constructor_common](
                  std::vector< float > coordinates)
                  -> revolve::brain::FakeLightSensor *
          {
            delete sensor_right;
...
BOOST_AUTO_TEST_CASE(run_multiple_times)
{
          ...
          -> revolve::brain::FakeLightSensor *
  {
    if (coordinates.size() not_eq 3)
    {
      std::cerr
              << "ERROR! COORDINATES TO THE LIGHT "
                      "CONSTRUCTOR ARE THE WRONG SIZE!"
              << std::endl;
    }
...
testSUPGBrainPhototaxis::testSUPGBrainPhototaxis(
...
void testSUPGBrainPhototaxis::update(
        const std::vector< revolve::gazebo::MotorPtr > &motors,
        const std::vector< revolve::gazebo::SensorPtr > &sensors,
        double t,
        double step)
